  ...
  describe RespondToMobileRequests do
    ...
    describe "#is_mobile_request?" do
      it "should be false for a normal request" do
        allow(subject).to receive_message_chain(:request, :user_agent).and_return('some mozilla')
        expect(subject.send(:is_mobile_request?)).to be_falsy
      end

      Mobylette::MobileUserAgents.new.call.to_s.split('|').each do |agent|
        agent.gsub!('\\', '')
        it "should be true for the agent #{agent}" do
          allow(subject).to receive_message_chain(:request, :user_agent).and_return(agent)
=begin
          if subject.send(:is_mobile_request?)
            p "TRUE: #{agent}"
          else
            p "FALSE: #{agent}"
          end
=end
    ...
    describe "#stop_processing_because_xhr?" do
      ...
      context "this is a xhr request" do
        before(:each) do
          allow(subject).to receive_message_chain(:request, :xhr?).and_return(true)
        end
    ...
    describe "#handle_mobile" do
      ...
      context "respond_as_mobile? is true" do
        before(:each) do
          ...
          allow(@request).to receive(:format=).and_return { |new_value| @format = new_value }
          allow(subject).to receive(:request).and_return(@request)
    ...
    describe "#request_device?" do
      it "should match a device" do
        allow(subject).to receive_message_chain(:request, :user_agent).and_return('very custom browser WebKit')
      ...
      it "should match an android phone" do
        allow(subject).to receive_message_chain(:request, :user_agent).and_return('This is Android browser Mobile')
        expect(subject.send(:request_device?, :iphone)).to be_falsey
    ...
    describe "#set_mobile_format" do
      ...
      context "not matching format in fallback chain" do
        it "should return :mobile" do
          subject.mobylette_options[:fallback_chains] = { html: [:html, :htm], mp3: [:mp3, :wav, :mid] }
          allow(subject).to receive_message_chain(:request, :user_agent).and_return("android")
